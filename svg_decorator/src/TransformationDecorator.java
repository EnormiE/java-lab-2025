public class TransformationDecorator extends ShapeDecorator {
    ...
    dostosowując przy tym różne jego cechy. Każda metoda buildera, poza build dodaje nową cechę do naszego docelowego obiektu,
    podobnie jak dekorator, a nawet sensowniej by było umieścić nasze poprzednie dekoratory w builderze!
    Możemy to porównać do składania zamówienia na jakiś samochód, wybiaramy sobie po kolei silnik, skrzynie biegów
    ...
    public static class Builder {
        ...
        public Builder translate(Vec2 translation) {
            this.transform += String.format("translate(%s %s) ", translation.x(), translation.y());
            return this;
        }

        public Builder rotate(float angle, Vec2 center) {
            this.transform += String.format("rotate(%s %s %s) ", angle, center.x(), center.y());
            return this;
        }

        public Builder scale(Vec2 scaleFactor) {
            this.transform += String.format("scale(%s %s) ", scaleFactor.x(), scaleFactor.y());
            return this;
        }
